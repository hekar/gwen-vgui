...
MButton::MButton(vgui::Panel *parent, const char *panelName, const char *text, vgui::Panel *pActionSignalTarget, const char *pCmd ) :
...
void MButton::ApplySettings( KeyValues *inResourceData )
{
	...
	if ( !command )
	{
		Error( "No Concommand set for Button! %s\n", GetName() );
		SetEnabled( false );
		return;
	}
